...
data = np.load('Bivariatesup_OU_more(nu=10,a=3,b=0.05,B=-0.1,an=1.95).npz',allow_pickle=True)
...
'''
print("selected_x_components",selected_x_components)
print("selected_y_components",selected_y_components)'''


mu_estimated = np.zeros((num_paths, 2, 1), dtype=float)
sig_estimated = np.zeros((num_paths, 2, 2), dtype=float)
...
#print(data_moments(selected_x_components, selected_y_components))
    ...
    mean_data, var_data, autocov_data = data_moments(selected_x_components[i], selected_y_components[i])
 ...
 #   print(err_vec(selected_x_components, selected_y_components, mu_init, sigma_init,an_init))
    ...
    args=(selected_x_components[i], selected_y_components[i], W_hat)
...
#print(criterion_wrapper(params_init, *args))
    ...
    print('mu_GMM1',i+1,'=', mu_GMM1, ' sigma_GMM1' ,i+1,'=',sigma_GMM1, ' an_GMM1',i+1,'=', an_GMM1)

    mean_data, var_data, autocovariance_data = data_moments(selected_x_components[i], selected_y_components[i])
    err1 = err_vec(selected_x_components[i], selected_y_components[i], mu_init, sigma_init, an_init)
    print('Mean of points ',i+1,'=', mean_data, ', Variance of points ',i+1,'=', var_data, ', Autocovariance of points ',i+1,'=', autocovariance_data)
    print('Mean of model ',i+1,'=', mean_model, ', Variance of model ',i+1,'=', var_model, ', Autocovariance of model ',i+1,'=', autocov_model)
    ...
    err2 = err_vec(selected_x_components[i], selected_y_components[i], mu_GMM2, sigma_GMM2, an_GMM2)
    ...
    mu_estimated[i] = mu_GMM2.reshape(2, 1)
    sig_estimated[i] = sigma_GMM2.reshape(2, 2)
    print(sig_estimated[i])
    a_n_estimated[i] = an_GMM2

print('Estimations of mu:', mu_estimated)
print('Estimations of sig:', sig_estimated)
print('Estimations of a_n:', a_n_estimated)

print('Estimations of mu of x component:', np.mean(mu_estimated[:, 0, 0]))
print('Estimations of mu of y component:', np.mean(mu_estimated[:, 1, 0]))
print('Mean of estimations of sig of x component:', np.mean(sig_estimated[:, 0, 1]))
